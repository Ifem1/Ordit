...
export default function HumanReviewListPage() {
  ...
  const load = async (address?: string) => {
    ...
    if (connected) {
      ...
      const all = await getRequestsForOrganizations(orgs.map((org) => org.id));
      setRequests(all.filter((req) => ["NEEDS_REVIEW", "NEEDS_REVISION"].includes(req.status)));
    }
    setLoading(false);
  };
  ...
  if (!wallet && !loading) {
    return (
      <Card className="max-w-lg">
        ...
        <p className="text-sm text-slate-400 mb-5">Review queues are derived from contract state.</p>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-400">Insight audits requiring expert review before final action</p>
      </div>
            ...
            <Link key={req.id} href={`/review/${req.id}`}>
              <Card hover className="flex items-center justify-between py-4">
                ...
                <div className="flex items-center gap-3 shrink-0">
                  <Badge variant={req.status === "NEEDS_REVIEW" ? "warning" : "muted"}>{req.status.replace(/_/g, " ")}</Badge>
